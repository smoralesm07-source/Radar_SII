...
CREATE TABLE IF NOT EXISTS risk_signals (
  ...
  confidence VARCHAR,
  why_flagged VARCHAR,
  recommended_checks VARCHAR,
  source_record_id VARCHAR,
  generated_at TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_company_year_entity_year ON company_year(entity_id, commercial_year);
CREATE INDEX IF NOT EXISTS idx_company_year_region ON company_year(region, commercial_year);
...
CREATE INDEX IF NOT EXISTS idx_ownership_society ON ownership_edges(entity_id);
CREATE INDEX IF NOT EXISTS idx_ownership_partner_entity ON ownership_edges(partner_entity_id);
CREATE INDEX IF NOT EXISTS idx_risk_signals_entity ON risk_signals(entity_id);
CREATE INDEX IF NOT EXISTS idx_risk_signals_type ON risk_signals(signal_type, period);
